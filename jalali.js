...
Jalali.prototype.convert = function (type, date) {
	date = date.split(this.format[1]).map(function (item) {
		return parseInt(item);
	});
	var format = this.format.split(this.format[1]),
	    r = {},
	    month = 1,
	    months = {
	...
	date = (function () {
		...
		date.forEach(function (item, index) {
			return d[format[index]] = item;
		});
		return d;
	})();
	if (type == 'AD') {
		var difference = -226899,
		    to = 'jalali';
	...
	r.d = (function () {
		var additionalDays = ((date.y - 1) * 365 + months[type].slice(0, date.m - 1).reduce(function (a, b) {
			return a + b;
		}, 0) + date.d + Math.floor((date.y - 1) / 4) + difference - Math.floor((r.y - 1) / 4)) % 365;
...
typeof module !== 'undefined' ? module.exports = Jalali : undefined.Jalali = Jalali;